...
def apply_redshift_on_spectrum(spflam, splam, z, z0=0):
...
class SevenDT:
	...
	def get_CMOS_IMX455_QE(self, path_table='../util/sony.imx455.qhy600.dat'):
	...
	def calculate_response(self, group='sdt_default'):
		s = self.sky_table
		filterset = self.filterset
		...
		for ii, (cwl, filtername) in enumerate(zip(filterset['cwl'], self.filterNameList)):
			...
			if ii%2 == 0:
				plt.text(cwl, 1.1*resp_sys[indx_resp].max()*1e2, filtername, horizontalalignment='center', size=10)
			elif ii%2 == 1:
				plt.text(cwl, 1.1*resp_sys[indx_resp].max()*1e2, filtername, horizontalalignment='center', size=10)
	...
	def get_synphot2obs(self, spflam, splam, z, z0=0, figure=False):
		...
		if figure:
			...
			plt.scatter(outbl['lam'], outbl['magobs'], c=outbl['snr'], marker='s', edgecolors='k', s=100, label='obs')
			cbar = plt.colorbar()
